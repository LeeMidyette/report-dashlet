 ...
 * Introduce cambios manuales para solucionar el problema con el sort que considera Strings
 * en lugar de enteros, y también pasa parámetros adicionales que llegan desde opts a axis
 *
...
Raphael.fn.g.linechart = function (x, y, width, height, valuesx, valuesy, opts) {
    ...
    if (opts.axis) {
        ...
        +ax[0] && axis.push(this.g.axis(x + gutter, y + gutter, width - 2 * gutter, minx, maxx, opts.axisxstep || Math.floor((width - 2 * gutter) / 20), 2,opts.labelsX));
        +ax[1] && axis.push(this.g.axis(x + width - gutter, y + height - gutter, height - 2 * gutter, miny, maxy, opts.axisystep || Math.floor((height - 2 * gutter) / 20), 3,opts.labelsY));
        +ax[2] && axis.push(this.g.axis(x + gutter, y + height - gutter, width - 2 * gutter, minx, maxx, opts.axisxstep || Math.floor((width - 2 * gutter) / 20), 0,opts.labelsX));
        +ax[3] && axis.push(this.g.axis(x + gutter, y + height - gutter, height - 2 * gutter, miny, maxy, opts.axisystep || Math.floor((height - 2 * gutter) / 20), 1,opts.labelsY));
    }
    var lines = this.set(),
        symbols = this.set(),
        line;
